<script lang="ts">
	...
	import { toggleShowPassword } from '$lib/utils/toggleShowPassword';

	let newPassword: string = '';
	let passwordInvalid: boolean = false;
	let el: HTMLInputElement | null;
	let elment: Element | null | undefined;
	let baseURL: string = '';
	...
	function goToRequestPasswordResetTokenPage() {
		goto(`${$page.url.origin}/accounts/reset_password_reset_link`);
	}
...
<div class="wrapper">
	<form action="">
		<div class="top">
			<h4>Create new Passwoed</h4>
			<p>Your new password should be at least 6 charcters long</p>
		</div>
		<div class="middle" class:short_password_error={passwordInvalid}>
			<label for="password">Enter new password</label>
			<div class="input" on:focusin|preventDefault|stopPropagation={handleFocusin}>
				<input
					type="password"
					name="password"
					id="password"
				...
				/>
				<div class="see_password">
					<i class="las la-eye" on:click|preventDefault|stopPropagation={toggleShowPassword} />
				</div>
			</div>
			<small>Password must be at least 6 charcters long</small>
		</div>
